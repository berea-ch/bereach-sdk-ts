 ...
 */
export function contactsDiscard(
  client: BereachCore,
  request: operations.DiscardContactsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.DiscardContactsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.DiscardContactsRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/contacts/bulk")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.DiscardContactsResponse$inboundSchema),
