...
import { BereachDefaultError } from "../models/errors/bereach-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | BereachDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | BereachDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new BereachDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new BereachDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
