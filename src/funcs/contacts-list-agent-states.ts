 ...
 */
export function contactsListAgentStates(
  client: BereachCore,
  request?: operations.ListAgentStatesRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ListAgentStatesResponse,
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.ListAgentStatesRequest$outboundSchema),
        value,
      ),
  ...
  const path = pathToFunc("/agent-state")();

  const query = encodeFormQuery({
    "keysOnly": payload?.keysOnly,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListAgentStatesResponse$inboundSchema),
