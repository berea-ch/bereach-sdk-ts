...
The people who currently work at one named organisation, read through the connected LinkedIn account. This is the exact current-employee list, so it is the way to answer a request about a particular organisation's people, including one asking for whoever holds a role there: give the organisation and put the role in `keywords`. A free-text search of the open web cannot answer that question and returns people holding the same role elsewhere.
