 ...
 */
export function contactsDeleteCampaign(
  client: BereachCore,
  request: operations.DeleteCampaignRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.DeleteCampaignResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.DeleteCampaignRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/contacts/campaigns/{campaignId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.DeleteCampaignResponse$inboundSchema),
