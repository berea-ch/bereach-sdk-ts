 ...
 */
export function contactsCampaignStatusTransition(
  client: BereachCore,
  request: operations.CampaignStatusTransitionRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CampaignStatusTransitionResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.CampaignStatusTransitionRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/contacts/campaigns/{campaignId}/status")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CampaignStatusTransitionResponse$inboundSchema),
