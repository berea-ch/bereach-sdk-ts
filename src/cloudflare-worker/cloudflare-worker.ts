...
export class BereachMCP extends McpAgent<Env, State, Props> {
  server!: McpServer;
  ...
  getSDK() {
    ...
    const sdk = new BereachCore({
      debugLogger: {
        log: (...args) => console.log(...args),
        group: (...args) => console.group(...args),
        groupEnd: (...args) => console.groupEnd(...args),
      },
      security: async () => ({ token: getHeader("authorization") }),
    });
    return sdk;
  }
}
...
export default {
  async fetch(request: Request, env: Env, ctx: any) {
    ...
    if (url.pathname === "/sse" || url.pathname.startsWith("/sse/")) {
      return BereachMCP.serveSSE("/sse", { binding: "BEREACH_MCP" }).fetch(
        request,
        env,
        ctx,
      );
    ...
    if (url.pathname === "/mcp") {
      return BereachMCP.serve("/mcp", { binding: "BEREACH_MCP" }).fetch(
        request,
        env,
        ctx,
      );
