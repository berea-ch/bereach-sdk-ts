 ...
 */
export function contactsPatchAgentState(
  client: BereachCore,
  request: operations.PatchAgentStateRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.PatchAgentStateResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.PatchAgentStateRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/agent-state/{key}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.PatchAgentStateResponse$inboundSchema),
